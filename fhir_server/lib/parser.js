...
var clinical_document_effective_time = ''
...
var valueFix = function (node) {
    ...
    }else if (node.attributes['type'] === 'BL'){
        var tmp = node.attributes.value
        value = {
            'value': tmp
        }
...
var makeAndStoreObservation = function (patient) {
    ...
    var observation = getResource('Observation', {
        'id': 'Observation/' + get_uuid().toString(),
        'subject': {
            'reference': patient.id
        }
    });
        ...
        this._self = {
            ...
            representedOrganization: function (node) {
                ...
                var organization = getResource('Organization', {
                    'id': 'Organization/' + get_uuid().toString()
                });
        ...
        this._self = {
            ...
            assignedEntity: function (node) {
                var practitioner = getResource('Practitioner', {
                    'id': 'Practitioner/' + get_uuid().toString()
                });
    ...
    this.manufacturedProduct = function (node) {
        ...
        case 'MedicationAdministration':
            ...
            var medication = getResource('Medication', {
                'id': 'Medication/' + get_uuid().toString()
            });
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.22.4.44': //Plan of care activity observation
            ...
            var procedureRequest = getResource('Procedure', {
                'id': 'Procedure/' + get_uuid().toString(),
                'subject': {
                    'reference': patient.id
                }
            });
        ...
        case '2.16.840.1.113883.10.20.22.4.17':
            ...
            if (!resource.prescription) {
                ...
                medicationOrder = getResource('MedicationOrder', {
                    'id': 'MedicationOrder/' + get_uuid().toString()
                });
...
var EntryRelationshipMedication = function (typeCode, medicationAdministration) {
    ...
    this.observation = function (node) {
        ...
        var condition = getResource('Condition', {
            'id': 'Condition/' + get_uuid().toString(),
            'patient': {
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.22.4.52': //Immunization activity
            ...
            var immunization = getResource('Immunization', {
                'id': 'Immunization/' + get_uuid().toString(),
                'patient': {
                    'reference': patient.id
                }
            });
        ...
        case '2.16.840.1.113883.10.20.22.4.16': //Medication activity
            ...
            var _medicationAdministration = getResource('MedicationAdministration', {
                'id': 'MedicationAdministration/' + get_uuid().toString(),
                'patient': {
                    'reference': patient.id
                },
                'prescription': {
...
var Act = function (resource, sectionCode) {
    var templateId = [];
    var claim;
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.22.4.3': //Problem concern act
            ...
            var condition = getResource('Condition', {
                ...
                'dateRecorded': clinical_document_effective_time
            });
            //var sectionCode =  {
            //   "system": "urn:oid:2.16.840.1.113883.3.143.1",
            //   "code": "NOT AVAILABLE",
            //   "display": "肝移植术后状态"
            // }
            //sectionCode = null 
            if (sectionCode){
                condition['category']['coding'].push(sectionCode)
            }
        ...
        case '2.16.840.1.113883.10.20.22.4.39':
            ...
            //Convert to procedure
            var procedureRequest = getResource('Procedure', {
                //'id': 'ProcedureRequest/' + get_uuid().toString(),
                'id': 'Procedure/' + get_uuid().toString(),
                'subject': {
                    'reference': patient.id
                }
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.22.4.49': //Encounter activities
            ...
            var encounter = getResource('Encounter', {
                'id': 'Encounter/' + get_uuid().toString(),
                'patient': {
                    'reference': patient.id
                }
            });
    ...
    this.subject = function (node) {
        if (_.contains(templateId, '2.16.840.1.113883.10.20.22.4.45')) {
            ...
            /*var familyMemberHistory = getResource('FamilyMemberHistory', {
                'id': 'FamilyMemberHistory/' + get_uuid().toString(),
                'patient': {
                    'reference': patient.id
                }
            });
    ...
    this.component = function (node) {
        ...
        if (_.any(templateId, function (value) {
            ...
            /*var clinicalImpression = getResource('ClinicalImpression', {
                'id': 'ClinicalImpression/' + get_uuid().toString(),
                'patient': {
                    'reference': patient.id
                }
            });
...
var Entry = function (resource,sectionCode) {
    var templateId = [];
    ...
    this.act = function (node) {
        ...
        if (isInContextOf(allergiesSection)) {
            ...
            var allergyIntolerance = getResource('AllergyIntolerance', {
                'id': 'AllergyIntolerance/' + get_uuid().toString(),
                'patient': {
                    'reference': patient.id
                }
            });
            ...
            proto.control.push(new Triplet(node, new Act(resource, sectionCode), templateId));
        }
    };
...
var Section = function (resource) {
    ...
    this.entry = function (node) {
        proto.control.push(new Triplet(node, new Entry(resource, sectionCode), templateId));
    };
...
var encompassingEncounter = function(encounter){
    var _encounter = encounter
    var patient = findPatient(proto.bundle);
    this.code = function (node) {
        if (node.attributes.code) {
                ensureProperty.call(_encounter, 'type', true).push({
                    'text': node.attributes.displayName
                });
            }
        //store2bundle(encounter, patient.id);
    ...
    this.effectiveTime = function(node) {
        this.low = function (node) {
            if (node.attributes.value){
                    ensureProperty.call(_encounter, 'period').start =node.attributes.value
                //store2bundle(encounter, patient.id);
            }
        };
        this.high = function (node) {
            ensureProperty.call(_encounter, 'period').end = node.attributes.value
...
encompassingEncounter.prototype = proto;

var componentOf = function (node){
    var patient = findPatient(proto.bundle);
    this.encompassingEncounter = function(node){
        var encounter = getResource('Encounter', {
                'id': 'Encounter/' + get_uuid().toString(),
                'subject': {
                    'reference': patient.id
        ...
        proto.control.push(new Triplet(node, new encompassingEncounter(encounter)));
    }
};
componentOf.prototype = proto;
...
var ClinicalDocument = function (patientId, parserStream) {
    var templateId = [];
    this.effectiveTime = function(node){
        clinical_document_effective_time = node.attributes.value
    };
    ...
    proto.composition = {
        ...
        'id': 'Composition/' + get_uuid().toString(),
        'section': []
    };
    ...
    this.recordTarget = function (node) {
        ...
        var patient = {
            ...
            //'id': 'Patient/' + ((this.patientId) ? this.patientId : get_uuid().toString())
            'id': ((this.patientId) ? this.patientId : get_uuid().toString())
        };
    ...
    //zuofeng code
    this.componentOf = function(node){

        proto.control.push(new Triplet(node, new componentOf(node)));
    };
